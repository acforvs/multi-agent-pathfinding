...
class CommBlock(nn.Module):
    def __init__(
        ...
        input_dim,
        output_dim=64,
        num_heads=DHC_CONFIG["communication"]["num_comm_heads"],
        num_layers=DHC_CONFIG["communication"]["num_comm_layers"],
    ):
        super().__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
    ...
    def forward(self, latent, comm_mask):
        ...
        for _ in range(self.num_layers):
            ...
            if len(comm_idx) == 1:
                ...
                ).view(DHC_CONFIG["batch_size"], num_agents, self.input_dim)
...
class Network(nn.Module):
    def __init__(
        self,
        input_shape=DHC_CONFIG["observation_shape"],
        cnn_channels=DHC_CONFIG["cnn_channels"],
        hidden_dim=DHC_CONFIG["hidden_dim"],
        max_comm_agents=DHC_CONFIG["max_comm_agents"],
        latent_dim=DHC_CONFIG["latent_dim"],
    ):
        super().__init__()

        self._batch_size = DHC_CONFIG["batch_size"]
        ...
        self.obs_encoder = nn.Sequential(
            nn.Conv2d(self.input_shape[0], cnn_channels, 3, 1),
    ...
    def step(self, obs, pos):
        ...
        if self.hidden is None:
            ...
            self.hidden = self.recurrent(latent, self.hidden)  # e''_i^{t - 1}
    ...
    def forward(self, obs, steps, hidden, comm_mask):
        ...
        assert comm_mask.size(2) == DHC_CONFIG["max_num_agents"]
        ...
        latent = latent.view(
            self._batch_size * num_agents, max_steps, self.latent_dim
        ).transpose(0, 1)
        ...
        hidden = hidden_buffer[torch.arange(self._batch_size), steps - 1]
