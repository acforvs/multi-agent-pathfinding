  ...
  vector<Node> neigh(Node nd, vector<map<Pt, Pt>> &nxt) {
    ...
    for (int i = 0; i < nd.n; i++) {
      ...
      for (int j = 0; j < nd.n; j++) {
        if (i == j) continue;
        if (abs(nd.pts[i].x - nd.pts[j].x) + abs(nd.pts[i].y - nd.pts[j].y) <=
            2) {
          coll = true;
          break;
        }
...
vector<vector<int>> Mstar_multi(vector<vector<int>> mp, vector<Pt> starts,
                                vector<Pt> fins) {
  Grid grid(mp);
  Node start(starts);
  Node finish(fins);
    ...
    for (int x = 0; x < m; x++) {
      for (int y = 0; y < m; y++) {
        if (grid.mp[x][y] == 0) {
          ...
          nxt[i][st] =
              nxtv;  // next vertex from st to finish[i] in optimal path
        }
      }
    }
  }
