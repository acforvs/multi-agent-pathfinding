...
struct Grid{
    ...
    bool collision(Node from, Node to) {
        ...
        int n = to.n;
        for (int i = 0; i < n; i++) {
            if (simple_collision.count(to.pts[i]))
                return true;
    ...
    vector<Node> neigh(Node nd, vector<map<Pt, Pt>>& nxt) {
        vector<vector<Pt>> res = {{}};
        for (int i = 0; i < nd.n; i++) {
            bool coll = false;
            for (int j = 0; j < nd.n; j++) {
                if (i == j) continue;
                if (abs(nd.pts[i].x - nd.pts[j].x) + abs(nd.pts[i].y - nd.pts[j].y) <= 2) {
                    coll = true;
                    break;
                }
            }
        ...
        for (auto pts: res) {
            ...
            if (!collision(nd, candidate))
                neighs.push_back(Node(pts));
        }
        return neighs;
    }
};
...
Node choose_node(set<Node, cmpByHeurDist> &nodes) {
    return *nodes.begin();
}
...
vector<vector<int>> Astar_multi(vector<vector<int>> mp, vector<Pt> starts, vector<Pt> fins) {
    Grid grid(mp);
    Node start(starts);
    Node finish(fins);
    ...
    while (!reachable.empty()) {
        ...
        if (eq(node, finish))
            return make_path(node, pr);

        explored.insert(node);
        reachable.erase(node);

        auto neigs_nodes = grid.neigh(node, nxt);
...
vector<vector<int>> Mstar_multi(vector<vector<int>> mp, vector<Pt> starts, vector<Pt> fins) {
    Grid grid(mp);
    Node start(starts);
    Node finish(fins);
...
int main(int argc, char* argv[]) {
    ...
    auto res = Mstar_multi(mp, starts, fins);
