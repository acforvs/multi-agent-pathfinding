...
class Environment:
    def __init__(
        ...
        self.last_actions = np.zeros(
            (self.num_agents, 5, 2 * obs_radius + 1, 2 * obs_radius + 1), dtype=np.bool
        )
    ...
    def get_heuri_map(self):
        dist_map = (
            np.ones((self.num_agents, *self.map_size), dtype=np.int32) * 2147483647
        )
        for i in range(self.num_agents):
            open_list = list()
            ...
            while open_list:
                ...
                right = x, y + 1
                if (
                    right[1] < self.map_size[1]
                    and self.map[right] == 0
                    and dist_map[i, x, y + 1] > dist + 1
                ):
                    dist_map[i, x, y + 1] = dist + 1
